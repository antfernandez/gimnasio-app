...
**Sitio público (`/g/[slug]`, Sprint 5/7):** no tiene policy de SELECT propia para el rol `anon` sobre `gimnasios`/`sitios` — la única vía de lectura pública es el RPC `obtener_sitio_publico`, `SECURITY DEFINER`, que no depende de RLS. Durante el diagnóstico del 404 en producción (Sprint 16) se crearon dos policies ad-hoc (`gimnasios: lectura publica del sitio`, `sitios: lectura publica si publicado`) directo en el SQL Editor para investigar; resultaron innecesarias y exponían `sitios.contenido_borrador` al rol `anon`, así que se revirtieron en `supabase/migrations/0012_rollback_rls_sitio_publico.sql`. La causa real del 404 era de renderizado (Cache Components horneando el `notFound()` en el shell prerenderizado), no de RLS — ver `app/g/[slug]/page.tsx` y `.claude/specs/gimnasioappfixsitiopublico (Ajustes para sp15).md`. `fetchSitio` ahí usa `"use cache"` + `cacheTag(`sitio:${slug}`)`; la Server Action `publicarSitio` (`app/protected/sitio/actions.ts`) invalida ese tag con `updateTag` en cada publicación.
